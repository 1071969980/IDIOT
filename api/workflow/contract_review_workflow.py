...
from api.load_balance import DEEPSEEK_REASONER_SERVICE_NAME, QWEN_PLUS_SERVICE_NAME, LOAD_BLANCER
...
async def contract_review_workflow(
    task_id: uuid4,
    contract_content_dict: dict[str, str],
    review_entrys: list[ReviewEntry],
    stance: str,
) -> ReviewWorkflowResult:
    with logfire.span("Contract Review Workflow::review_entrys"):
        # load template
        ...
        user_prompt = template.render(
            review_entrys=review_entrys, stance=stance, **contract_content_dict
        )
        messages = [
            ChatCompletionSystemMessageParam(
                role="system", content="你是一个助手，请按照用户要求完成合同审查。"
            ),
            ChatCompletionUserMessageParam(role="user", content=user_prompt),
        ]

        logfire.info(
            "Contract Review Workflow::review entrys::user prompt",
        ...
        contract_review_response_thinking = "".join(
            [
        ...
        logfire.info(
            "Contract Review Workflow::review entrys::response",
    ...
    with logfire.span("Contract Review Workflow::format result"):
        template = JINJA_ENV.get_template(
            AvailableTemplates.ContractReviewJsonFormatter.value
        )
        ...
        logfire.info(
            "Contract Review Workflow::format result::response",
    ...
    for _res in workflow_res_unaligned.result:
        workflow_res.result.append(
            ReviewResult(
                entry=review_entrys[
                    _res.entry_id - 1
                ],  # LLM count entry_id start from 1
                risks=_res.risks,
            ),
        )
