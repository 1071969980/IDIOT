...
UPDATE_USER_MESSAGE_STATUS_BY_UUIDS = sql_statements["UpdateUserMessageStatusByUuids"]
UPDATE_USER_MESSAGE_SESSION_TASK_BY_UUIDS = sql_statements["UpdateUserMessageSessionTaskByUuids"]
...
class _U2AUserMessageUpdate:
    ...
    message_id: int
    fields: Dict[
        Literal["user_id", "session_id", "seq_index", "message_uuid", "message_type", "content", "status", "session_task_id"],
        Union[str, int]
    ]
...
async def insert_user_message(message_data: _U2AUserMessageCreate) -> int:
    """插入新U2A用户消息
...
async def get_next_user_message_seq_index(session_id: str) -> int:
    """获取会话的下一条消息序列索引
...
async def check_user_message_exists(message_id: int) -> bool:
    """检查消息是否存在
...
async def get_user_message_by_id(message_id: int) -> Optional[_U2AUserMessage]:
    """获取消息信息
...
async def get_user_messages_by_session(session_id: str) -> list[_U2AUserMessage]:
    """根据会话ID获取所有消息
...
async def get_user_messages_by_user(user_id: str) -> list[_U2AUserMessage]:
    """根据用户ID获取所有消息
...
async def get_user_message_field(
    message_id: int,
    field_name: Literal["id", "user_id", "session_id", "seq_index", "message_uuid", "message_type", "content", "status", "session_task_id", "created_at", "updated_at"]
...
async def get_user_message_fields(
    message_id: int,
    field_names: list[Literal["id", "user_id", "session_id", "seq_index", "message_uuid", "message_type", "content", "status", "session_task_id", "created_at", "updated_at"]]
) -> Optional[Dict[
    Literal["id", "user_id", "session_id", "seq_index", "message_uuid", "message_type", "content", "status", "session_task_id", "created_at", "updated_at"],
    Union[int, str]
]]:
    """获取消息的多个字段值
...
async def delete_user_message(message_id: int) -> bool:
    """删除消息
...
async def delete_user_messages_by_session(session_id: str) -> bool:
    """删除指定会话的所有消息
...
async def update_user_message_status_by_uuids(
    message_uuids: list[str],
    new_status: Literal["agent_working_for_user", "waiting_agent_ack_user", "complete", "error"]
) -> int:
    """根据消息UUID批量更新消息状态
    ...
    async with ASYNC_SQL_ENGINE.connect() as conn:
        result = await conn.execute(
            text(UPDATE_USER_MESSAGE_STATUS_BY_UUIDS),
            {
...
async def update_user_message_session_task_by_uuids(
    message_uuids: list[str],
    session_task_id: Optional[str]
) -> int:
    """根据消息UUID批量更新消息的session_task_id
    ...
    async with ASYNC_SQL_ENGINE.connect() as conn:
        result = await conn.execute(
            text(UPDATE_USER_MESSAGE_SESSION_TASK_BY_UUIDS),
            {
